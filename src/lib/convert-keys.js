  ...
  Object.keys(resultantObject).forEach(function (key) {
    ...
    if (value instanceof Array) {
      for (var i = 0, length = value.length; i < length; i++) {
        resultantObject[newKeyName][i] = convertKeys(
          value[i],
          map[mapKey][const_arrayKeyPropertyName],
          reverseMap
        );
      }
    } else if (typeof value == "object") {
      resultantObject[newKeyName] = convertKeys(
        value,
        map[mapKey][const_objectValuePropertyName],
        reverseMap
      );
    }
  });
