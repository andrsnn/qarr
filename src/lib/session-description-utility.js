...
const baseConversion = require('./base-number-convert').default;
const convertKeys = require('./convert-keys').default;
...
var SDPUtility = {
  ...
  generateBlobContentsFromDescription: function (
    description,
    /*optional*/ shouldShortenKeys
  ) {
    var default_context = {
      payloads: -1,
      ip: "",
    ...
    if (applicationMediaDesc) {
      ...
      context.ip = applicationMediaDesc.connection
        ? applicationMediaDesc.connection.ip
        : null;
      ...
      var fingerprint =
        (applicationMediaDesc.fingerprint
          ? applicationMediaDesc.fingerprint.hash
          : null) || parsedSDP.fingerprint.hash;
      ...
      context.candidates = (function () {
        ...
        applicationMediaDesc.candidates.forEach(function (
          candidate,
          index,
          array
        ) {
  ...
  restoreSessionDescription: function (contextFromBlob, type) {
    ...
    var template_sdp = function (context) {
      var sdpTransformData = {
        ...
        fingerprint: {
          hash: (function () {
            // Convert it back
            var hash_base16 = baseConversion.baseConvert(
              context.fingerprint,
              64,
              16
            );
            // Separate each hex byte by a colon
            var hashByteColonSep = hash_base16.replace(
              /([0-9A-F]{2})(?=.)/g,
              "$1:"
            );
            ...
            candidates: (function () {
              ...
              context.candidates.forEach(function (candidate, index, array) {
                ...
                candidate.foundation =
                  Math.round(Math.random() * 50000) + 50000;
      ...
      var browserInfo = window.navigator.appVersion.match(
        /(Chrom[e|ium])\/(\d*)/
      );
      var isChrome = false;
    ...
    if (type == null) {
      console.warn(
        "No type provided when restoring SDP from blob. This will most likely cause a failure to open a datachannel"
      );
    }
    ...
    if (this.debug) {
      console.log(
        "Restored compiled sdp context",
        compiledTransformSDPDescData
      );
    }
